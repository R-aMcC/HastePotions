...
public class Haste2 extends BrewAction {

    @Override
    public void brew(BrewerInventory inventory, ItemStack item, ItemStack ingredient) {
        if(item.getType() == Material.POTION && Utils.isAwkwardPotion(item) && ingredient.getType() == Material.GOLD_INGOT) {
            Utils.setPotion(inventory, 1, false);
        }
    }
}
